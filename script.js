/* ==============================================================
   AeroPulse – Wind-Tunnel Dashboard
   • NACA 0012 airfoil with real angle-of-attack rotation
   • Particle flow field
   • Live data from ESP32 (JSON endpoint)
   ============================================================== */

const ESP32_URL = 'http://192.168.4.1/data';   // <-- change to your ESP32 IP
...
async function fetchData() {
    try {
        ...
        console.warn('ESP32 fetch error → using fallback animation', e);
        // fallback: gentle sine-wave angle
        angleOfAttack = 5 + Math.sin(Date.now()*0.0005)*12;
        el.aoa.innerHTML = `${angleOfAttack.toFixed(1)}<span class="unit">°</span>`;
        el.angleControl.textContent = `${angleOfAttack.toFixed(1)}°`;
    }
}
